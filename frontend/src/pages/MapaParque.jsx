...
function MapaParque() {
  ...
  const cargarGrafo = useCallback(async () => {
    try {
      ...
      // Calcular posiciones incluyendo nodos nuevos
      setPosiciones(calcularPosiciones(data.nodos));
    } catch {
  ...
  useEffect(() => {
    ...
    grafo.aristas.forEach((arista) => {
      const posOrigen = posiciones[arista.origen]; // ← posiciones dinámicas
      const posDestino = posiciones[arista.destino]; // ← posiciones dinámicas
      if (!posOrigen || !posDestino) return;

      const enRuta =
        rutaActiva.includes(arista.origen) &&
    ...
    grafo.nodos.forEach((nodo) => {
      const pos = posiciones[nodo.id]; // ← posiciones dinámicas
      if (!pos) return;
  ...
  }, [grafo, rutaActiva, origen, destino, posiciones]); // ← agregar posiciones
  ...
  if (cargando)
    return <div className="cargando">Cargando mapa del parque...</div>;
        ...
        <div className="canvas-container">
          ...
          <div className="leyenda">
            <span className="ley-item">
              <span
                className="ley-color"
                style={{ background: COLORES.nodoOrigen }}
              />
              Origen
            </span>
            <span className="ley-item">
              <span
                className="ley-color"
                style={{ background: COLORES.nodoDestino }}
              />
              Destino
            </span>
            <span className="ley-item">
              <span
                className="ley-color"
                style={{ background: COLORES.nodoRuta }}
              />
              En ruta
            </span>
            <span className="ley-item">
              <span
                className="ley-color"
                style={{ background: COLORES.nodoNormal }}
              />
              Nodo
            </span>
          </div>
        ...
        <div className="panel-control">
          ...
          {tab === "dijkstra" && (
            <form onSubmit={handleDijkstra} className="form-grafo">
              ...
              {resultado && (
                <div
                  className={`resultado ${resultado.encontrado ? "exito" : "error"}`}
                >
          ...
          {tab === "bfs" && (
            <form onSubmit={handleBFS} className="form-grafo">
              <h3>Recorrido BFS</h3>
              <p className="form-desc">
                Explora todos los nodos del parque por niveles desde un punto de
                inicio.
              </p>
              <div className="form-group">
              ...
              {bfsResultado && (
                <div className="resultado exito">
                  ...
                  <p className="res-distancia">
                    Total: <strong>{bfsResultado.totalNodos} nodos</strong>{" "}
                    recorridos
                  </p>
                </div>
              )}
